...
export function Candle({ color, x, y, scale = 1, delay = 0, locked = false, visualLevel = 1 }: Props) {
  const c = CANDLE_COLOR_HEX[color];
  const flameOpacity = visualLevel <= 1 ? 0.68 : visualLevel === 2 ? 0.84 : 1;
  const glowOpacity = visualLevel <= 1 ? 0.1 : visualLevel === 2 ? 0.18 : visualLevel >= 5 ? 0.3 : 0.22;
  return (
    <g className="cake-candle" transform={`translate(${x} ${y}) scale(${scale})`} opacity={locked ? 0.45 : 1}>
      ...
      {!locked && (
        <g className="cake-flame" style={{ animationDelay: `${delay}ms`, opacity: flameOpacity, transformOrigin: "0px -16px" }}>
          <ellipse cx="0" cy="-16" rx="20" ry="22" fill="#facc15" opacity={glowOpacity} className="cake-flame-glow" />
          <ellipse cx="0" cy="-14" rx="13" ry="15" fill="#fef3c7" opacity={Math.min(0.42, glowOpacity + 0.14)} />
